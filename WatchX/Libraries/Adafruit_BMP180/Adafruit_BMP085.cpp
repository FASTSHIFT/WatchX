...
uint8_t Adafruit_BMP085::read8(uint8_t a) {
  ...
  BMP_WIRE.beginTransmission(BMP085_I2CADDR); // start transmission to device 
#if (ARDUINO >= 100)
  BMP_WIRE.write(a); // sends register address to read from
#else
  BMP_WIRE.send(a); // sends register address to read from
#endif
  BMP_WIRE.endTransmission(); // end transmission
  
  BMP_WIRE.beginTransmission(BMP085_I2CADDR); // start transmission to device 
  BMP_WIRE.requestFrom(BMP085_I2CADDR, 1);// send data n-bytes read
#if (ARDUINO >= 100)
  ret = BMP_WIRE.read(); // receive DATA
#else
  ret = BMP_WIRE.receive(); // receive DATA
#endif
  BMP_WIRE.endTransmission(); // end transmission
...
uint16_t Adafruit_BMP085::read16(uint8_t a) {
  ...
  BMP_WIRE.beginTransmission(BMP085_I2CADDR); // start transmission to device 
  BMP_WIRE.requestFrom(BMP085_I2CADDR, 2);// send data n-bytes read
#if (ARDUINO >= 100)
  ret = BMP_WIRE.read(); // receive DATA
  ret <<= 8;
  ret |= BMP_WIRE.read(); // receive DATA
...
#if (ARDUINO >= 100)
  BMP_WIRE.write(a); // sends register address to read from
  BMP_WIRE.write(d);  // write data
#else
  BMP_WIRE.send(a); // sends register address to read from
  BMP_WIRE.send(d);  // write data
#endif
  BMP_WIRE.endTransmission(); // end transmission
}
void Adafruit_BMP085::updateAll()
{
    temperature = readTemperature();
    pressure = readPressure();
    sealevelPressure = readSealevelPressure();
